...
if [[ -x "$(pwd)/llvm-3.7.0.install/bin/clang-3.7" && \
      -x "$(pwd)/llvm-3.7.0.install/bin/opt" && \
      -e "$(pwd)/llvm-3.7.0.install/lib/libclang.so.3.7" ]] ; then

    echo "Setting variables:"
    echo "LLVM_3_7_0_BIN=\"$(pwd)/llvm-3.7.0.install/bin\""
    export LLVM_3_7_0_BIN="$(pwd)/llvm-3.7.0.install/bin"
    echo "LLVM_3_7_0_LIB=\"$(pwd)/llvm-3.7.0.install/lib\""
    export LLVM_3_7_0_LIB="$(pwd)/llvm-3.7.0.install/lib"
    echo "PATH=\"$LLVM_3_7_0_BIN:\$PATH\""
    export PATH="$LLVM_3_7_0_BIN:$PATH"
